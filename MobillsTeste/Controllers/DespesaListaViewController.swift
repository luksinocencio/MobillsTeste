...
class DespesaListaViewController: UIViewController {
    ...
    func recuperarDespesas() {
        DB_REF_DESPESA.order(by: "timestamp", descending: true).getDocuments { snapshot, error in
            if let error = error {
...
extension DespesaListaViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = FormatarDinheiro.shared.getFormattedValue(of: despesas[indexPath.row].valor, withCurrency: "R$")
        cell.detailTextLabel?.text = despesas[indexPath.row].descricao
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            let deleteAction = UIAlertAction(title: "Excluir despesa", style: .default) { (action) in
                ...
                DB_REF_DESPESA.whereField("id", isEqualTo: self.despesas[indexPath.row].id).getDocuments { snapshot, error in
                    if let error = error {
